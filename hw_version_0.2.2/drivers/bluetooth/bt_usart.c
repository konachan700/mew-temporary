...
static void __mew_bluetooth_config(void);
...
static volatile uint8_t _mew_comm_state = 0;
...
//static const mew_bt_at_command _mew_at_is_cofigured_pin = { 0, 0, "AT+PASS4343", "+OK" };
...
static void __mew_bluetooth_config(void) {
    ...
    if (errcode == MEW_BT_CS_ERROR) {
        errcode = mew_bluetooth_execute_at(&_mew_at_is_set_name);
        if (errcode != MEW_BT_CS_OK) {
            mew_debug_die_with_message("can't change bluetooth name");
        }
    }

//    errcode = mew_bluetooth_execute_at(&_mew_at_is_cofigured_pin);
//    if (errcode != MEW_BT_CS_OK) {
//        mew_debug_die_with_message("can't set default pin");
//    }
    
    
    errcode = mew_bluetooth_execute_at(&_mew_at_is_transparent);
    if (errcode != MEW_BT_CS_OK) {
        mew_debug_die_with_message("can't switch bluetooth to transparent mode");
    }
