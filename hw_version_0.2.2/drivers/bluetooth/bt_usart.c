...
#define MEW_BT_AT_BUFFER_TIME_MS	100
...
static char _mew_bt_at_buffer[MEW_BT_AT_BUFFER_SIZE];
static volatile uint32_t _mew_bt_at_buffer_counter = 0;

static uint8_t _mew_adapter_mac[6];

unsigned int mew_bluetooth_handler(void) {
	char bt_at_buffer[MEW_BT_AT_BUFFER_SIZE];

	if (_mew_bt_cmd_channel_state == MEW_BT_STATE_TRANSPARENT) return 0;
	if ((_mew_bt_cmd_timer < mew_get_millis()) && (_mew_bt_cmd_timer != 0)) {
		//nvic_disable_irq(MEW_BLUETOOTH_IRQ);
		...
		case MEW_BT_ATCMD_GET_MAC:
			if (strstr(bt_at_buffer, "MAC")) {
				...
				mew_debug_print_hex_ex("BT MAC: ", (char*)(bt_at_buffer + 5), 12);
			}
			break;
...
void MEW_BLUETOOTH_ISR(void) {
    if (USART1_SR & USART_SR_RXNE) {
    	...
    	case MEW_BT_STATE_AT:
    		if ((_mew_bt_cmd_timer == 0) || (_mew_bt_cmd_timer > mew_get_millis())) {
    			_mew_bt_at_buffer[_mew_bt_at_buffer_counter] = (uint8_t) MEW_BLUETOOTH_DMA_DR;
				_mew_bt_at_buffer_counter++;
				if (_mew_bt_at_buffer_counter >= MEW_BT_AT_BUFFER_SIZE) {
					memset(_mew_bt_at_buffer, 0, MEW_BT_AT_BUFFER_SIZE);
					_mew_bt_at_buffer_counter = 0;
				}
				_mew_bt_cmd_timer = mew_get_millis() + MEW_BT_AT_BUFFER_TIME_MS;
    		} else {
//    			memset(_mew_bt_at_buffer, 0, MEW_BT_AT_BUFFER_SIZE);
//    			_mew_bt_at_buffer[0] = (uint8_t) MEW_BLUETOOTH_DMA_DR;
//    			_mew_bt_at_buffer_counter = 1;
//    			_mew_bt_cmd_timer = 0;
    		}
    		break;
    	case MEW_BT_STATE_TRANSPARENT:
    		mew_comm_add_byte(MEW_DRIVER_ID_BLUETOOTH, (uint8_t) MEW_BLUETOOTH_DMA_DR);
    		break;
    	}
    }
}
